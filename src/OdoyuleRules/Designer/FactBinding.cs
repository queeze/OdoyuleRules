...
namespace OdoyuleRules.Designer
{
    ...
    public class FactBinding<T> :
        Binding<T>
        where T : class
    {
        readonly RuleConfigurator _configurator;

        public FactBinding(RuleConfigurator configurator)
        {
        ...
        public Binding<T, TRight> Join<TRight>()
            where TRight : class
        {
            return new FactBinding<T, TRight>(_configurator);
        }
    ...
    public class FactBinding<TLeft, TRight> :
        ...
        where TRight : class
    {
        ...
        {
            IEnumerable<RuleConditionConfiguratorImpl<TLeft>> conditions = expression.ParseConditions()
                .Select(x => new RuleConditionConfiguratorImpl<TLeft>(x));
        ...
        {
            IEnumerable<RuleConditionConfiguratorImpl<TRight>> conditions = expression.ParseConditions()
                .Select(x => new RuleConditionConfiguratorImpl<TRight>(x));
