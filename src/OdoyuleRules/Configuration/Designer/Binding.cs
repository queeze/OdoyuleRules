...
namespace OdoyuleRules.Configuration.Designer
{
    ...
    public interface RuleDefinitionConfigurator<T> :
        RuleDefinitionConfigurator
        where T : class
    {
        RuleDefinitionConfigurator<T> When(Expression<Func<T, bool>> expression);

        Binding<T, TRight> Join<TRight>()
            where TRight : class;

        RuleDefinitionConfigurator<T> Then(Action<ThenConfigurator<T>> configureCallback);
    }
